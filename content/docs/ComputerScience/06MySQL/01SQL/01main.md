...
title: 基础语法学习
weight: 10
data: 2025-11-08T13:55:00+08:00
---
...
> 这个子句主要是起到一个辅助功能，用来辅助诸如`SELECT`、`UPDATE`、`DELETE`等语句，帮助它们确定作用的范围。
...
> [!NOTE] `NOT` > `AND` > `OR`
> 这三个是`WHERE`子句中用来处理逻辑关系的关键字，用法和优先级和逻辑代数中相同，当然也可以使用括号来改变优先级。
...
> [!NOTE] `IN`
> 这个子句用来查询属性值属于一个集合的所有行，另外还经常和**子查询**一起使用。
...
> [!NOTE] `BETWEEN a AND b`
> 这个子句用来查询属性值在一个范围内的所有行，注意是闭区间。
...
> [!NOTE] `LIKE`
> 这个子句用来进行模糊查询，其中`%`表示任意字符串（可以为空），`_`表示任意字符(不可为空)，上面两个模式串的区别是:
> - '王%'可以搜到`王大力`和`王五`。
> - '王_'只能搜到`王五`
...
> [!DANGER] 这样是错误的！
> `null`不等于任何值包括`null`，这个子句应该写成`WHERE level is null`，相反我们可以用`WHERE level is NOT null`来查找填写了等级的玩家。
> 当然，如果读者非常偏爱等于符号，也可以用`WHERE level <=> null`来表示某个值为空。
> 
> 此外，空字符串`''`也不等于`null`。

#### ORDER 

顾名思义，将查询结构安装升序（`ASC`或不加）或者降序排列（`DESC`），这里也可以枚举多个，排在前面的排序规则优先级高，此外也可以用列的序号代替列名，比如如果level是第5列，那么在命令中可以用`5`来代替`level`。
...
#### HAVING 过滤器
> 用于过滤查询结果，通常和`GROUP BY`、`ORDER BY`组合使用。
...
> [!SUCCESS] 练习
> 统计各个姓氏的人的数量，按照数量降序排列，只显示人数大于5的姓氏。
>
> `提示`：这里可能会用到`SUBSTR`这个聚合函数，请自行学习。

#### LIMIT 限制查询结果数量
拿刚刚的练习举例，如果我们只想要前三名，就可以这样：

```mysql
SELECT SUBSTR(name, 1, 1), COUNT(SUBSTR(name, 1, 1)) FROM player
GROUP BY SUBSTR(name, 1, 1)
HAVING SUBSTR(name, 1, 1) >= 5
ORDER BY SUBSTR(name, 1, 1) DESC
LIMIT 3
```

此外，LIMIT还可以有两个参数，如下面的两个`3`，第一个是`偏移量`表示从第4名开始列，第二个表示列3个。
...
#### UNION 合并查询结果(并集)
> 功能上没什么好说的，注意会自动去重，如果不想去重，就加一个`ALL`关键字
```mysql
...
## 子查询
> 有的时候我们会使用另一个查询结果来作为当前查询的条件，这个时候就要用到子查询。
...
> [!DANGER] AS
> 给列取别名，这样查询结果就会显示为你自定义的别名。

当然也可以用来创建新表，比如我们可以把原表中的某些玩家数据用来创建新表
```mysql
...
## 表关联
> 表关联用来查询多个表中的数据，关联的表之间必须有相同的字段，一般会使用表的主键和外键来关联。主要分为：`INNER JOIN`、`LEFT JOIN`、`RIGHT JOIN`。
>
> **这里只展示两个表的关联**。

### 内连接
> 返回两表都有的数据，不存在的字段用null填充。

这里假设存在一个`equip`表，存在`id`, `name`, `player_id`三个字段，这样我们可以使用玩家id这个字段来关联两个表。
```mysql
SELECT * FROM player
INNER JOIN equip
...
### 左连接
> 返回左表中所有的数据和右表中匹配的数据，左表中不存在的字段用NULL填充。
```mysql
SELECT * FROM player
LEFT JOIN equip
ON player.id = equip.player_id;
...
### 笛卡儿积
> ==笛卡儿积是指两个集合中所有可能的元素有序组成的新集合==，如果把表连接的条件子句去除，就会输出笛卡儿积。
...
> [!NOTE] 思考
> 其实从这里也能看出，==表连接的本质是笛卡儿积加条件过滤==，我们把条件过滤去掉就是笛卡儿积了。

## 索引
> 索引是一种用来提高查询效率的数据结构，它可以帮助我们快速定位到我们想要的数据，如果没用索引，就只能从头开始遍历所有的数据，直到找到满足条件的数据为止，当数据非常少的时候，这样没什么问题，但是当数据量极大，查询耗时就会非常恐怖。
...
以上是创建索引的两种语法格式，注意方括号内是可选参数，索引分三类：`唯一索引`、`全文索引`、`空间索引`，我们通常对主键或者经常用来查询的字段创建索引，也就是where后面放的字段，这样可以提高查询效率。
...
## 视图
> 视图是一种虚拟的表，它本身并不包含数据，而是作为一个查询语句，保存至数据字典中，当我们查询视图的时候，它会根据视图定义来动态生成数据。
```mysql
